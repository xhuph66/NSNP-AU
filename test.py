...
for i in range(30):
	...
	dataset_name1 = "sunspot"
	# Download Data
	series = read_csv('sunspot.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
...
# Construct an LSTM network model and train it, the number of samples: 1, the number of cyclic training: 100, the number of neurons in the GSNP layer is 8
	...
	np.savetxt('./NSNP-AU/'+dataset_name+'/prediction/' + str(count) + '_'+dataset_name1+'.csv', predictions, delimiter=',')
	np.savetxt('./NSNP-AU/'+dataset_name+'/origin/' + str(count) + '_'+dataset_name1+'.csv', raw_values[-1174:], delimiter=',')
