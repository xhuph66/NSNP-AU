...
from keras.layers import NSNPAU
...
def fit_lstm(train, batch_size, nb_epoch, neurons):
    ...
    model.add(NSNPAU(neurons, batch_input_shape=(batch_size, X.shape[1], X.shape[2]), stateful=True))#Peephole
...
cout=0
for i in range(30):
	dataset_name="LORENZ１"
	dataset_name1= "lorenz"
	# 加载数据
	#series = read_csv('lorenz1.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
	series = read_csv('lorenz1.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
	#series = read_csv('lorenz3.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
	#series = read_csv('gas.csv', header=0, parse_dates=[0], index_col=0, squeeze=True)
...
# Where X is the ith row from 0 to -1, and the shape is (1,) a one-dimensional array containing one element; y is the ith row and the penultimate column, which is a number;
	...
	np.savetxt('./NSNP-AU/'+dataset_name+'/prediction/' + str(count) + '_'+dataset_name1+'.csv', predictions[-1872,:,:], delimiter=',')
	np.savetxt('./NSNP-AU/'+dataset_name+'/origin/' + str(count) + '_'+dataset_name1+'.csv', raw_values[-1872,:,:], delimiter=',')
